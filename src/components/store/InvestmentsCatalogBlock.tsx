...
function InvestmentCompactCard({
  item,
  onOpen,
}: {
  item: InvestmentCatalogItem;
  onOpen: (item: InvestmentCatalogItem) => void;
}) {
  return (
    <div
      className={cn(
...
function InvestmentDialog({
  ...
  item: InvestmentCatalogItem | null;
  open: boolean;
      ...
      >
          ...
          <div className="flex w-full min-h-0 shrink-0 flex-col gap-2 overflow-hidden border-b border-primary/15 px-3 py-3 sm:gap-2.5 sm:px-4 sm:py-3.5 lg:w-[min(100%,400px)] lg:max-w-[400px] lg:border-b-0 lg:border-e xl:w-[420px] xl:max-w-[420px]">
            <DialogHeader className="shrink-0 space-y-1.5 text-right">
              <div className="flex flex-wrap items-center justify-end gap-2">
                {item.taken ? (
                  ...
                  <Badge className="border-primary/30 bg-primary/15 font-display text-[11px] text-primary">مفتوح</Badge>
                )}
                <Badge variant="outline" className="font-display text-[11px]">
                  <Layers className="me-1 h-3 w-3" /> {item.sector}
                </Badge>
                {item.partnersAllowed ? (
                  <Badge variant="outline" className="border-amber-400/40 bg-amber-500/10 text-amber-700 font-display text-[11px]">
                    <Handshake className="me-1 h-3 w-3" /> يقبل الشراكة
                  </Badge>
                ) : null}
              </div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <div className="rounded-xl border border-primary/20 bg-gradient-to-br from-primary/[0.08] to-transparent p-2">
                  <p className="text-[9px] font-display text-muted-foreground">المساهمة</p>
                  <p className="mt-0.5 font-latin-display text-base font-bold tabular-nums text-primary sm:text-lg" dir="ltr">
                    {usd.format(item.priceUsd)}
                  </p>
                </div>
                <div className="rounded-xl border border-amber-400/30 bg-amber-500/[0.08] p-2">
                  <p className="text-[9px] font-display text-muted-foreground">العائد المتوقع</p>
                  <p className="mt-0.5 font-display text-[11px] font-bold leading-snug text-amber-700 sm:text-xs">
                    {item.expectedReturn}
                  </p>
                </div>
              </div>
            ...
            <DialogDescription asChild>
              <div className="shrink-0 rounded-xl border border-primary/12 bg-muted/30 p-2.5 text-right">
                <p className="mb-1 font-display text-[10px] tracking-wider text-primary/90">عن الفرصة</p>
        ...
        <div className="flex min-h-[52px] shrink-0 flex-col gap-2 border-t border-primary/15 bg-muted/25 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-start sm:gap-6 sm:px-4 sm:py-2.5">
          <div className="flex w-full justify-start sm:w-auto sm:shrink-0">
            <StoreOrderTicketCTA
              taken={item.taken}
              buttonLabel="طلب الاستثمار"
              confirmSummary={item.name}
              subject={`استثمار — ${item.name}`}
              productDetailsBody={[
                "طلب من صفحة المتجر (الاستثمارات).",
                "",
                `المعرّف: ${item.id}`,
                `الاسم: ${item.name}`,
                ...
                `القطاع: ${item.sector}`,
                `المساهمة المعروضة: ${usd.format(item.priceUsd)}`,
                `العائد المتوقع: ${item.expectedReturn}`,
                `يقبل الشراكة: ${item.partnersAllowed ? "نعم" : "لا"}`,
              ]
                .filter(Boolean)
                .join("\n")}
          ...
          <p className="min-w-0 flex-1 text-right text-[11px] leading-snug text-muted-foreground sm:text-xs">
            الأرقام المعروضة تمثيلية داخل الرول بلاي ولا تُعتبر استشارة مالية حقيقية.
          </p>
        </div>
      </DialogContent>
    </Dialog>
...
export function InvestmentsCatalogBlock() {
  const { investments } = useInvestmentsContent();
  const visible = investments.filter((i) => !i.hidden);
  const [selected, setSelected] = useState<InvestmentCatalogItem | null>(null);
  const [dialogOpen, setDialogOpen] = useState(false);

  const open = (i: InvestmentCatalogItem) => {
    setSelected(i);
    setDialogOpen(true);
  };
    ...
    <>
      <section className="mx-auto max-w-[1600px]">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6 xl:grid-cols-4 xl:gap-5">
          {visible.map((i) => (
            <InvestmentCompactCard key={i.id} item={i} onOpen={open} />
          ))}
        </div>
      </section>
      <InvestmentDialog item={selected} open={dialogOpen} onOpenChange={handleDialog} />
    </>
  );
}

export default InvestmentsCatalogBlock;
