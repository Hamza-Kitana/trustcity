...
import { useInvestmentsContent } from "@/contexts/InvestmentsContentContext";
...
function suggestId(name: string, existingIds: string[]): string {
  ...
  const fromLatin = asciiOnly
    ...
    .toLowerCase()
    .slice(0, 40);
  const base = fromLatin.length >= 2 ? fromLatin : `inv-${crypto.randomUUID().slice(0, 8)}`;
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate font-mono text-[10px] text-slate-600" dir="ltr">
            {item.id} · ${item.priceUsd} · {item.sector}
          </p>
        </div>
        {item.hidden ? (
...
const emptyItem = (): InvestmentCatalogItem => ({
  id: "",
  name: "",
  nameEn: "",
...
const InvestmentsEditorPage = () => {
  const { user } = useAuth();
  const { investments, reorder, add, update, remove } = useInvestmentsContent();
  ...
  useEffect(() => {
    if (!selectedId && investments.length > 0) setSelectedId(investments[0].id);
    if (selectedId && !investments.some((i) => i.id === selectedId)) {
      setSelectedId(investments[0]?.id ?? null);
    }
  }, [investments, selectedId]);

  useEffect(() => {
    const sourceIds = investments.map((i) => i.id);
    setOrderedIds((prev) => {
  ...
  const displayed = useMemo(() => {
    ...
    return orderedIds.map((id) => byId.get(id)).filter((c): c is InvestmentCatalogItem => Boolean(c));
  }, [investments, orderedIds]);

  const filtered = displayed.filter((i) => {
    const q = searchQuery.trim().toLowerCase();
    const matchesSearch = !q || `${i.name} ${i.id} ${i.priceUsd} ${i.sector} ${i.location}`.toLowerCase().includes(q);
    const matchesStatus = statusFilter === "all" ? true : statusFilter === "taken" ? i.taken : !i.taken;
    return matchesSearch && matchesStatus;
  });

  const hasPendingOrderChanges =
    orderedIds.length === investments.length && orderedIds.some((id, i) => id !== investments[i]?.id);
  ...
  const openEdit = (i: InvestmentCatalogItem) => {
    setIsNew(false);
    setEditing({ ...i, galleryUrls: [...i.galleryUrls], highlights: [...i.highlights] });
    setGalleryText(i.galleryUrls.join("\n"));
    setHighlightsText(i.highlights.join("\n"));
    setDialogOpen(true);
  };
  ...
  const saveDialog = () => {
    ...
    const full: InvestmentCatalogItem = {
      id: editing.id,
      name,
      ...
      expectedReturn: editing.expectedReturn.trim() || "—",
      location: editing.location.trim() || "—",
      highlights,
      partnersAllowed: editing.partnersAllowed,
      taken: editing.taken,
      hidden: !!editing.hidden,
    };

    if (isNew) {
      const existingIds = investments.map((i) => i.id);
      const id =
      ...
      add({ ...full, id });
      setSelectedId(id);
      appendActivityLog(user?.username ?? "—", "استثمار: إضافة", full.name);
      toast.success("تمت الإضافة");
    } else {
      update(editing.id, { ...full, id: editing.id });
      appendActivityLog(user?.username ?? "—", "استثمار: تعديل", full.name);
      ...
      <div className="flex flex-col gap-4 text-right sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="flex items-center justify-end gap-2 font-display text-2xl font-bold text-slate-900">
            <TrendingUp className="h-7 w-7 text-rose-700" />
            مدير الاستثمار في المدينة
          </h1>
          <p className="mt-2 max-w-xl text-sm text-slate-600">
            أضف فرص الاستثمار (مطاعم، كازينو، محطات وقود…) التي تظهر في{" "}
            <a
              href="/store?tab=investment"
              target="_blank"
              rel="noreferrer"
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          <Button
            ...
            onClick={() => setPageVisible("investments", !visibility.pages.investments)}
          >
            {visibility.pages.investments ? "إخفاء قسم الاستثمار" : "إظهار قسم الاستثمار"}
          </Button>
          {hasPendingOrderChanges ? (
            <Button
              ...
              className="border-rose-200 bg-white text-rose-700 hover:bg-rose-50 hover:text-rose-800"
              onClick={() => {
                const working = investments.map((i) => i.id);
          ...
          <Button type="button" className="bg-[#36164f] text-white hover:bg-[#2f1344]" onClick={openNew}>
            <Plus className="ms-1 h-4 w-4" /> فرصة جديدة
          </Button>
        </div>
      </div>
      ...
      <div className="space-y-3 rounded-2xl border border-rose-200/80 bg-white/85 p-4 shadow-[0_18px_44px_-28px_rgba(127,29,29,0.45)]">
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="text-right">
            <Label htmlFor="inv-search" className="text-slate-700">
              بحث (الاسم، القطاع، الموقع، السعر)
            </Label>
            <Input
              id="inv-search"
              value={searchQuery}
          ...
          <div className="text-right">
            ...
            <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as "all" | "taken" | "available")}>
              ...
              <SelectContent dir="rtl" className="border-rose-200 bg-white text-slate-900">
                ...
                <SelectItem value="available">المفتوحة فقط</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
        ...
        >
          <div className="shrink-0 border-b border-rose-200 bg-[radial-gradient(ellipse_100%_120%_at_50%_0%,rgba(244,63,94,0.22),transparent_58%)] px-6 pb-4 pt-14 sm:px-8 sm:pt-16">
            <DialogHeader className="space-y-1.5 text-right sm:text-right">
              <DialogTitle className="font-display text-xl font-bold text-slate-900 sm:text-2xl">
                {isNew ? "فرصة استثمارية جديدة" : "تعديل الفرصة"}
              </DialogTitle>
              <DialogDescription className="text-sm leading-relaxed text-slate-600">
                املأ بيانات الفرصة، ثم احفظ — يظهر التحديث فوراً في صفحة المتجر.
              </DialogDescription>
            </DialogHeader>
            {!isNew ? (
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-5 sm:px-8">
            <EditorDialogSection title="التعريف">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-slate-600">القطاع</Label>
                  <Input
                    className={cn(editorDialogInputClass, "mt-1.5 border-rose-200 bg-white text-slate-900")}
                    placeholder="مثل: خدمات وضيافة، ترفيه، بنية تحتية"
            ...
            <EditorDialogSection title="السعر والحالة">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-slate-600">السعر/المساهمة (USD)</Label>
                  <Input
                    type="number"
                    min={0}
                    className={cn(editorDialogMonoClass, "mt-1.5 border-rose-200 bg-white text-slate-900")}
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-slate-600">العائد المتوقع (نص)</Label>
                  <Input
                    className={cn(editorDialogInputClass, "mt-1.5 border-rose-200 bg-white text-slate-900")}
                    placeholder='مثل: "5–10% شهرياً"'
                    value={editing.expectedReturn}
                    onChange={(e) => setEditing((p) => ({ ...p, expectedReturn: e.target.value }))}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-slate-600">يقبل الشراكة؟</Label>
                  <Select
                    value={editing.partnersAllowed ? "yes" : "no"}
                    onValueChange={(v) => setEditing((p) => ({ ...p, partnersAllowed: v === "yes" }))}
                  >
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-slate-600">حالة الفرصة</Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent dir="rtl">
                      <SelectItem value="no">مفتوحة</SelectItem>
                      <SelectItem value="yes">محجوزة</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
            ...
            <EditorDialogSection title="المزايا الرئيسية (سطر لكل ميزة)">
              <Textarea
...
export default InvestmentsEditorPage;
